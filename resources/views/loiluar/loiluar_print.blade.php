...
<head>
    <title>Loi-{{ $loiluar->nomor_loiluar }}</title>
    <style type="text/css">
        @page {
            margin: 0px;
        }
...
<body>
    ...
    <div class="container">
        ...
        @foreach ($loiluars as $lo)
            <table style="width: 100%;">
                <tr>
                    <td style="vertical-align: top; width: 50%;">
                        <table style="width: 100%; padding: 0px;line-height: 1.4;">

                            <tr>
                                <td>No</td>
                                <td>: {{ $loiluar->nomor_loiluar }}</td>
                            </tr>
                            <tr>
                                <td>Date</td>
                                <td>: {{ $loiluar->tanggal_loiluar }}</td>
                            </tr>
                            <tr>
                                <td>To</td>
                                <td>: {{ $lo->nama }}</td>
                            ...
                            <tr>
                                ...
                                <td>
                                    @php
                                        $emails = array_map('trim', explode(',', $lo->email));
                                    @endphp
            ...
            <div class="content">
                ...
                <p style="text-align: justify;">
                    We are pleased to place this Letter of intent for this components referring to your last price on
                    Purchase Order : <b>{{ $loiluar->nomor_po }}</b> with the following conditions:
                </p>
                ...
                @foreach ($loiluar->details->chunk(5) as $chunk)
                    <table class="table" align="center">
                        <thead>
                            <tr>
                                ...
                                <th style="text-align: center">Unit Price({{ $symbol }})</th>
                                <th style="text-align: center">Total Price({{ $symbol }})</th>
                        ...
                        <tbody>
                            @foreach ($chunk as $item)
                                @php
                                    $harga_per_unit = is_numeric($item->harga_per_unit) ? $item->harga_per_unit : 0;
                                    $harga_per_unit_imss = is_numeric($item->harga_per_unit_imss)
                                        ? $item->harga_per_unit_imss
                                        : 0;
                                    $loiluar_qty = is_numeric($item->loiluar_qty) ? $item->loiluar_qty : 0;

                                    $total = $loiluar_qty * $harga_per_unit;
                                    $total_imss = $loiluar_qty * $harga_per_unit_imss;
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: center">{{ $item->loiluar_qty }}</td>
                                    <td style="text-align: center">{{ $item->satuan }}</td>
                                    <td
                                        style="word-wrap: break-word; max-width: 90px; overflow: hidden; text-overflow: ellipsis; text-align: left;">
                                        {{ number_format($harga_per_unit, 2) }}
                                    </td>
                                    <td
                                        style="word-wrap: break-word; max-width: 90px; overflow: hidden; text-overflow: ellipsis; text-align: left;">
                        ...
                        @if ($loop->last)
                            <tfoot>
                                <tr>
                                    ...
                                    <td style="text-align: left; font-weight: bold;">
                                        {{ number_format($total_penawaran, 2) }}
                                    </td>
                                </tr>
                            </tfoot>
                        @endif
                ...
                <p>
                    Kindly consider the proposed price negotiation with the following notes:
                </p>
                ...
                <table class="tabel-2" style="width:100%;">
                    <tr>
                        <td>
                            @php
                                $data = $loiluar->keterangan_loiluar ?? '';
                                $items = explode("\n", $data);
                            @endphp
                ...
                <p style="text-align: justify;">
    Thank you for your favourable consideration to the above and looking forward
    to having officially confirmation on the terms and conditions stipulated above.
</p>
                    ...
                    <tr>
                        <td style="text-align: center;">
                            <div style="display: inline-block; margin-left: -150px;">
                                <b><u>{{ $loiluar->signature_vendor }}</u></b>
                            </div>
                        </td>
                        <td style="text-align: center; vertical-align: bottom">
                            <b style="text-decoration: underline;">
                                {{ $loiluar->signature_imss }}
                        </td>
                    </tr>
                </table>
            </div>
