                    ...
                    <table id="table" class="table table-sm table-bordered table-hover table-striped">
                        <thead>
                            <tr class="text-center">
                                ...
                                <th>{{ __('Jenis Proyek') }}</th>
                                <th>{{ __('Alamat Proyek') }}</th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            'ttd_vendor' => $d->ttd_vendor,
                            'jabatan_vendor' => $d->jabatan_vendor,
                            'nama_proyek' => $d->proyek_name,
                            'jenis_proyek' => $d->jenis_proyek,
                            'alamat_proyek' => $d->alamat_proyek,
                            'no_po' => $d->no_po,
                            ...
                            <tr>
                                ...
                                <td class="text-center">{{ $data['jenis_proyek'] }}</td>
                                <td class="text-center">{{ $data['alamat_proyek'] }}</td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" id="save" action="{{ route('purchase_order.store') }}" method="post">
                    ...
                    <div class="form-group row">
                        <label for="jenis_proyek" class="col-sm-4 col-form-label">{{ __('Jenis Proyek') }}</label>
                        <div class="col-sm-8">
                            <select class="form-control" id="jenis_proyek" name="jenis_proyek">
                    ...
                    <div class="form-group row">
                        <label for="alamat_proyek" class="col-sm-4 col-form-label">{{ __('Alamat Proyek') }}</label>
                        <div class="col-sm-8">
                            <textarea class="form-control" name="alamat_proyek" id="alamat_proyek" rows="3"></textarea>
                        </div>
                    </div>
                    <div class="form-group row">
                    ...
                    <div class="form-group row">
                        <label for="ref_po" class="col-sm-4 col-form-label">{{ __('Referensi PO') }} </label>
                        <div class="col-sm-8">
                    ...
                    <div class="form-group row">
                        <label for="ppn" class="col-sm-4 col-form-label">{{ __('PPN') }} </label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="ppn" name="ppn">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="pph" class="col-sm-4 col-form-label">{{ __('PPH') }} </label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="pph" name="pph">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="ttd_vendor" class="col-sm-4 col-form-label">{{ __('Nama Terang Vendor') }} </label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="ttd_vendor" name="ttd_vendor">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="jabatan_vendor" class="col-sm-4 col-form-label">{{ __('Jabatan Vendor') }} </label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="jabatan_vendor" name="jabatan_vendor">
                        </div>
                    </div>
                </form>
            </div>
...
<script>
    ...
    function editPo(data) {
        ...
        $('#ppn').val(data.ppn);
        $('#pph').val(data.pph);
        $('#ttd_vendor').val(data.ttd_vendor);
        $('#jabatan_vendor').val(data.jabatan_vendor);
    ...
    $(document).on('click', '#edit_po_save', function() {
        var id = $(this).data('id');
        var id_po = $('#id_po').val(); // Ambil dari input hidden agar tidak null
        ...
        if (!id_po || isNaN(parseInt(id_po))) {
            toastr.error('ID PO tidak boleh kosong!');
            return;
        }
        if (!batas) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#po_tanggal').text(data.po.tgpo);
                $('#po_batas').text(data.po.btpo);
                $('#id_po').val(data.po.id); // Pastikan #id_po selalu diisi ulang setelah update
                $('#button-cetak-po').html('<i class="fas fa-print"></i> Cetak');
                $('#button-cetak-po').attr('disabled', false);
                var no = 1;
                var id_po = data.po.id;
                ...
                if (data?.po?.details?.length == 0) {
                    ...
                    $.each(data?.po?.details, function(index, value) {
                        ...
                        var html = '<tr>' +
                            ...
                            '" name="harga_per_unit' + id + '"></td>' +
                            '<td><input type="text" value="' + mata_uang +
                            '" class="form-control" id="mata_uang' + id + '" name="mata_uang' + id + '"></td>' +
                            '<td><input type="text" value="' + vat +
                            '" class="form-control" id="vat' + id + '" name="vat' + id + '"></td>' +
                            '<td>' + total + '</td>' +
                            '<td><button title="simpan" id="edit_po_save" type="button" class="btn btn-success btn-xs" data-id="' +
                ...
                $('#tabel-po').find('tr:first').remove();
                setTimeout(function() {
                    toastr.success('Berhasil menyimpan detail Purchase Order!');
                }, 200);
            },
            error: function(xhr) {
                ...
                if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                    ...
                    toastr.error(xhr.responseJSON?.message || 'Gagal menyimpan detail Purchase Order!');
                }
            }
        })
    });
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data.products, function(key, value) {
                    ...
                    var checkbox;
                    if (value.qty_po === null || value.qty_po === "" || value.qty_po >= 0) {
    ...
    function addToDetails() {
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#po_tanggal').text(data.po.tgpo);
                $('#po_batas').text(data.po.btpo);
                $('#id_po').val(data.po.id_po); // Pastikan #id_po selalu diisi ulang setelah update
                $('#button-cetak-po').html('<i class="fas fa-print"></i> Cetak');
                $('#button-cetak-po').attr('disabled', false);
                $('#tabel-po').empty();
                var no = 1;
