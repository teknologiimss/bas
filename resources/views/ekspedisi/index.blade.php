...
@section('content')
    ...
    <style>
        ...
        /* Responsive Mobile Layout (Card View for Phones) */
        .mobile-card-item {
            background: #ffffff;
                ...
                @if (session('success'))
                    <div id="success-alert" class="alert alert-dismissible fade show toast-alert" role="alert">
                        <div class="d-flex align-items-center p-2">
                            ...
                            <div>
                                <strong class="text-dark" style="font-size: 0.95rem;">Berhasil!</strong>
                ...
                <div class="card card-navy">
                    <div class="card-navy-header">
                        ...
                        <button type="button" class="btn btn-navy-primary" data-toggle="modal" data-target="#modalTambah">
                            <i class="fas fa-plus-circle mr-1"></i> Tambah Ekspedisi
                        </button>
                    </div>

                    <div class="card-body p-3 p-md-4">
                        {{-- VIEW UNTUK DESKTOP & TABLET --}}
                                ...
                                <tbody>
                                    @forelse($dokumens as $index => $item)
                                        <tr>
                                            ...
                                            <td><span class="badge badge-light p-2 border">{{ $item->diterima_oleh }}</span>
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                @else
                                                    <span class="badge badge-secondary badge-pill px-3 py-2">Tidak ada
                                                        file</span>
                                                @endif
                                            </td>
                        ...
                        {{-- VIEW UNTUK HP / MOBILE (CARD LIST) --}}
                        <div class="mobile-card-view d-md-none">
                            @forelse($dokumens as $index => $item)
                                <div class="mobile-card-item">
                                    <div class="d-flex justify-content-between align-items-center mb-2 pb-2 border-bottom">
                                        <span class="badge badge-primary px-2 py-1">#{{ $index + 1 }}</span>
                                    ...
                                    <div class="row text-small mb-3 bg-light p-2 rounded mx-0">
                                        <div class="col-6 px-1">
                                            ...
                                            <strong>{{ $item->diterima_oleh }}</strong>
                                        </div>
                                        <div class="col-6 px-1">
                                            <small class="text-muted d-block">Ket:</small>
                    ...
                    <div class="modal-content modal-content-navy shadow-lg">
                        <div class="modal-header modal-header-navy">
                            <h5 class="modal-title font-weight-bold" id="modalEditLabel{{ $item->id }}">
                                <i class="fas fa-edit mr-2"></i>Edit Ekspedisi Dokumen
                            </h5>
                        ...
                        <div class="modal-body p-4">
                            <div class="form-group">
                                <label class="font-weight-bold text-secondary">Nama Pengirim</label>
                                <input type="text" class="form-control bg-light"
                                    value="{{ $item->sender->name ?? 'N/A' }}" readonly>
                            </div>
                            <div class="form-group">
                                <label class="font-weight-bold text-secondary">Deskripsi</label>
                                <textarea name="deskripsi" class="form-control" rows="3" required>{{ old('deskripsi', $item->deskripsi) }}</textarea>
                            </div>
                            <div class="form-group">
                                <label class="font-weight-bold text-secondary">Diterima Oleh</label>
                            ...
                            <div class="form-group mb-0">
                                <label class="font-weight-bold text-secondary">Upload Dokumen Baru</label>
                                <input type="file" name="file_dokumen" class="form-control-file">
                                @if ($item->file_dokumen)
                                    <small class="form-text text-info mt-2">
                                        <i class="fas fa-info-circle mr-1"></i> File saat ini:
                                        <a href="{{ asset('storage/' . $item->file_dokumen) }}" target="_blank"
                        ...
                        <div class="modal-footer bg-light">
                            ...
                            <button type="submit" class="btn btn-navy-primary rounded-pill px-4">Simpan
                                Perubahan</button>
                        </div>
                    </div>
                </form>
                ...
                <div class="modal-content modal-content-navy shadow-lg">
                    <div class="modal-header modal-header-navy">
                        <h5 class="modal-title font-weight-bold" id="modalTambahLabel">
                            <i class="fas fa-plus-circle mr-2"></i>Tambah Ekspedisi Dokumen
                        </h5>
                        <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    ...
                    <div class="modal-body p-4">
                        ...
                        <div class="form-group mb-0">
                            <label class="font-weight-bold text-secondary">Upload Dokumen</label>
                            <input type="file" name="file_dokumen" class="form-control-file">
                        </div>
                    </div>
                    <div class="modal-footer bg-light">
                        <button type="button" class="btn btn-secondary rounded-pill px-4"
                            data-dismiss="modal">Batal</button>
                        <button type="submit" class="btn btn-navy-primary rounded-pill px-4">Simpan Data</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    {{-- Script Tambahan untuk Otomatis Menutup Alert Toast --}}
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (successAlert) {
                ...
                }, 4000); // Alert otomatis hilang setelah 4 detik
            }
        });
    </script>
@endsection
