    ...
    <div class="container py-4">
        <div class="card p-4 border-0 shadow-sm rounded-4 mb-4">
            <div class="d-flex justify-content-between align-items-center flex-wrap">
                <div>
                    <h3>📋 {{ $pompa->judul }}</h3>
                    <p class="text-muted mb-0">Jenis Perawatan: <strong>{{ $pompa->jenis_perawatan }}</strong> | No Pompa:
                        <strong>{{ $pompa->no_pompa }}</strong></p>
                </div>
                <div class="d-flex gap-2">
                    <a href="{{ route('pompa.index') }}" class="btn btn-secondary">Kembali</a>
                    <a href="{{ route('pompa.mobile', $pompa->id) }}" class="btn btn-success">📱 Input Inspeksi</a>
                    <a href="{{ route('pompa.print', $pompa->id) }}" target="_blank" class="btn btn-primary">🖨️ Cetak
                        PDF</a>
                </div>
            </div>
        </div>
