...
@section('content')
    ...
    <div class="container mt-4 mb-5">
        <h3 class="page-title mb-4">✏️ Edit Checksheet Pompa</h3>

        <form action="{{ route('pompa.update', $checksheet->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="card main-card p-4 mb-4">
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3 unscheduled-field"
                        ...
                        <label class="fw-bold">No. Form Unscheduled Pompa</label>
                        <input type="text" autocomplete="off" name="no_form_unscheduled" class="form-control"
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="date" name="tanggal_pelaksanaan" class="form-control"
                            value="{{ $checksheet->tanggal_pelaksanaan }}" required>
                    </div>
                    ...
                    <div class="col-md-4 mb-3 unscheduled-field"
                        ...
                        <select name="status_kondisi" class="form-select">
                            <option value="" disabled {{ is_null($checksheet->status_kondisi) ? 'selected' : '' }}>--
                                Pilih Status --</option>
            ...
            <div id="scheduled-container" class="scheduled-field"
                ...
                <div id="items-container">
                    @php $groupedItems = $checksheet->items->groupBy('uraian_pekerjaan'); @endphp
                    @foreach ($groupedItems as $uraian => $items)
                        ...
                        <div class="item-row border rounded p-3 mb-3">
                            ...
                            <button type="button" class="btn btn-info btn-sm text-white mt-2"
                                onclick="addDetail({{ $iIndex }})">➕ Tambah Aktivitas</button>
                        </div>
                    @endforeach
                </div>
                <button type="button" class="btn btn-primary mb-4" onclick="addItem()">➕ Tambah Uraian
                    Pekerjaan</button>
            </div>

            <button type="submit" class="btn btn-success save-btn">💾 Update Checksheet</button>
        </form>
    </div>
    ...
    <script>
        ...
        function addItem() {
            ...
            <div class="item-row border rounded p-3 mb-3">
                <div class="row mb-3">
                    <div class="col-md-2"><input type="text" name="items[${iIndex}][nomor]" class="form-control" placeholder="No"></div>
                    <div class="col-md-9"><input type="text" name="items[${iIndex}][uraian_pekerjaan]" class="form-control" placeholder="Uraian Pekerjaan"></div>
                    <div class="col-md-1"><button type="button" class="btn btn-danger w-100" onclick="removeItem(this)">✖</button></div>
                </div>
                <div id="details-${iIndex}"></div>
                <button type="button" class="btn btn-info btn-sm text-white mt-2" onclick="addDetail(${iIndex})">➕ Tambah Aktivitas</button>
            </div>`;
        ...
        function addDetail(iIndex) {
            ...
            let html = `
            <div class="row mb-2 detail-row align-items-center">
                <div class="col-md-7"><input type="text" name="items[${iIndex}][details][${dIndex}][aktivitas_pekerjaan]" class="form-control" placeholder="Aktivitas"></div>
                <div class="col-md-4"><input type="text" name="items[${iIndex}][details][${dIndex}][standar]" class="form-control" placeholder="Standar"></div>
                <div class="col-md-1 text-center"><button type="button" class="btn btn-danger w-100" onclick="removeDetail(this)">✖</button></div>
            </div>`;
