...
@section('custom-js')
    <script>
        ...
        function editBarang(data) {
            ...
            let id_barang = data.id_po ?? data.id_poluar;
            $('#edit-barang').find('#id_barang').val(data.id_barang);
        ...
        //     const id_po_real = data.id_po || data.id_poluar
        //     const type_po = data.hasOwnProperty('id_po') ? 'PO' : 'POLUAR'

        //     $.ajax({
        //         url: "{{ url('penerimaan_barang') }}" + "/" + id_po_real + "?type_po=" + type_po,
        //         type: "GET",
        //         dataType: "json",
        //         beforeSend: function() {
        ...
        //                         .no_po + '</td><td>' + value.nama + '</td><td>' + value
        //                         .kode_material + '</td><td>' +
        //                         value.uraian + '</td><td>' + value.spek + '</td><td>' + value
        //                         .qty + '</td><td>' + data.pr.nama_proyek + '</td><td>' +
        //                         terima_eks + '</td><td>' + belum_terima_eks +
                ...
                success: function(data) {
                    ...
                    if (data.pr.details.length == 0) {
                        ...
                        $.each(data.pr.details, function(key, value) {
                            ...
                            // Gunakan nullish coalescing agar default "-"
                            var qtyp = value.penerimaan ?? '-';
                            var ok = value.hasil_ok ?? '-';
                            var nok = value.hasil_nok ?? '-';
                            var terima_eks = value.diterima_eks ?? '-';
                            var belum_terima_eks = value.belum_diterima_eks ?? '-';

                            var editButton =
                                '<button type="button" class="btn btn-success btn-xs mr-1" data-row-id="' +
                                value.id + '" title="Edit" onclick="editRow(\'' + value.id + '\', \'' +
                                value.uraian + '\', \'' + value.spek + '\', \'' + value.id_pr +
                                '\', \'' + qtyp + '\', \'' + terima_eks + '\', \'' + belum_terima_eks +
                                '\',  \'' + value.no_po + '\',  \'' + data.pr.nama_proyek +
                                '\',  \'' + id_po_real +
                                '\')"><i class="fas fa-edit"></i></button>';
                            ...
                            $('#table-pr').append(
                                ...
                                '<td>' + value.spek + '</td>' +
                                '<td>' + value.qty + '</td>' +
                                '<td>' + data.pr.nama_proyek + '</td>' +
                                '<td>' + terima_eks + '</td>' +
                                '<td>' + belum_terima_eks + '</td>' +
                                '<td>' + editButton + '</td>' +
                                '</tr>'
                            );
                        });
        ...
        function updateData(formData) {
            $.ajax({
                url: "{{ url('products/lppb/editpenerimaan') }}",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
                ...
                success: function(data) {
                    ...
                    // toggle container
                    if ($('#detail-pr').find('#container-product').hasClass('d-none')) {
                        $('#detail-pr').find('#container-product').removeClass('d-none').addClass('col-5');
                        $('#detail-pr').find('#container-form').removeClass('col-12').addClass('col-7');
                        $('#button-tambah-produk').text('Kembali');
                    } else {
                        $('#detail-pr').find('#container-product').removeClass('col-5').addClass('d-none');
                        $('#detail-pr').find('#container-form').addClass('col-12').removeClass('col-7');
                        $('#button-tambah-produk').text('Tambah Item Detail');
                        clearForm();
                    }
                    ...
                    if (data.pr.details.length == 0) {
                        ...
                        $.each(data.pr.details, function(key, value) {
                            ...
                            var editButton =
                                '<button type="button" class="btn btn-success btn-xs mr-1" title="Edit" ' +
                                'onclick="editRow(\'' + value.id + '\', \'' +
                                value.uraian + '\', \'' + value.spek + '\', \'' + value.id_pr +
                                '\', \'' + qtyp + '\', \'' + terima_eks + '\', \'' + belum_terima_eks +
                                '\', \'' + value.no_po + '\', \'' + data.nama_proyek +
                                '\', \'' + data.pr.id_po_real + '\')">' +
                                '<i class="fas fa-edit"></i></button>';
