...
@section('custom-css')
    ...
    ===================================================== */
        ...
        input[type="submit"] {
            ...
            box-shadow: 0 6px 18px rgba(15, 34, 58, .25);
        }
        ...
        .btn:hover {
            ...
            box-shadow: 0 12px 28px rgba(15, 34, 58, .35);
    ...
    ===================================================== */
        ...
        table {
            ...
            border-radius: 14px;
            overflow: hidden;
            box-shadow: 0 12px 30px rgba(15, 34, 58, .15);
        }
        ...
        table tbody tr:hover {
            background: linear-gradient(135deg, #F7FBFF, #DCEAF8) !important;
            transform: translateY(-3px);
            box-shadow: 0 8px 18px rgba(15, 34, 58, .18);
        }
    ...
    ===================================================== */
        ...
        .modal-content {
            border-radius: 14px;
            overflow: hidden;
            box-shadow: 0 20px 45px rgba(15, 34, 58, .20);
        }
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form role="form" id="edit-registrasi-btn" action="{{ route('nomor_lppb.edit') }}"
                                ...
                                <input type="hidden" name="type_po" id="type_po"> <!-- tambahin -->
                                <div class="form-group row">
...
@section('custom-js')
    <script>
        ...
        function editBarang(data) {
            ...
            let id_prr = data.id_po ?? data.id_poluar;
            $('#edit-barang').find('#id_prr').val(id_prr);
            $('#edit-barang').find('#nomor_lppb').val(data.nomor_lppb);
            $('#edit-barang').find('#tanggal_lppb').val(data.tanggal_lppb);
            // tambahin type_po (misal: 'PO' atau 'PO-Luar')
            $('#edit-barang').find('#type_po').val(data.type_po);
        }
        ...
        //     const id_po = data.id_po || data.id_poluar
        //     const type_po = data.hasOwnProperty('id_po') ? 'PO' : 'POLUAR'

        //     $.ajax({
        //         url: "{{ url('products/lppb_detail') }}" + "/" + id_po + "?type_po=" + type_po,
        //         type: "GET",
        //         dataType: "json",
        //         beforeSend: function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.pr.details.length == 0) {
                        ...
                        $('#table-pr').empty();
                        $.each(data.pr.details, function(key, value) {
                            var qtyp = value.penerimaan || '-';
                            var ok = value.hasil_ok || '-';
                            var nok = value.hasil_nok || '-';
                            var terima_qc = value.diterima_qc || '-';
                            var belum_terima_qc = value.belum_diterima_qc || '-';
                            ...
                            if (value.tgl_diterima) {
                                ...
                                tgl_tampil = date.getDate() + ' ' + monthNames[date.getMonth()] + ' ' +
                                    date.getFullYear();
                                tgl_terima = value.tgl_diterima;
                            }
                            ...
                            var editButton =
                                ...
                                value.id + '" title="Edit" onclick="editRow(\'' + value.id + '\', \'' +
                                value.uraian +
                                '\', \'' + value.spek + '\', \'' + value.id_pr + '\', \'' + qtyp +
                                '\', \'' + ok + '\', \'' + nok + '\', \'' + value.no_po + '\', \'' +
                                data.pr.nama_proyek + '\', \'' + terima_qc + '\', \'' +
                                belum_terima_qc +
                                '\', \'' + tgl_terima + '\', \'' + data.pr.id_po_woi + '\')">' +
                                '<i class="fas fa-edit"></i></button>';

                            $('#table-pr').append('<tr><td>' + value.no_pr + '</td><td>' +
                                value.no_po + '</td><td>' + value.kode_material + '</td><td>' +
                                value.uraian + '</td><td>' + value.spek + '</td><td>' +
                                value.qty + '</td><td>' + value.satuan + '</td><td>' +
                                data.pr.nama_proyek + '</td><td>' + qtyp +
                                '</td><td>' + ok + '</td><td>' + nok +
                                '</td><td>' + terima_qc + '</td><td>' +
                                belum_terima_qc + '</td><td>' + tgl_tampil +
                                '</td><td>' + editButton + '</td></tr>');
                        });
                    }
                }
        ...
        function editRow(id, uraian, spek, id_pr, qtyp, ok, nok, no_po, nama_proyek, terima_qc, belum_terima_qc,
            tgl_terima, id_po) {
            console.log(id, uraian, spek, id_pr, qtyp, ok, nok, no_po, nama_proyek, terima_qc, belum_terima_qc, tgl_terima,
                id_po);
            resetForm();
