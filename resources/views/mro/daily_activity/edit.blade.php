...
@section('content')
        ...
        <form action="{{ route('mro.daily-activity.update', $daily->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card card-primary">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="status" class="form-control">
                            <option value="Open" {{ $daily->status == 'Open' ? 'selected' : '' }}>Open</option>
                            <option value="Closed" {{ $daily->status == 'Closed' ? 'selected' : '' }}>Closed</option>
                        </select>
                    </div>
                    ...
                    <h5><b>Lampiran Saat Ini</b></h5>
                    @forelse($daily->attachments as $lampiran)
                        <div class="border rounded p-2 mb-2 d-flex justify-content-between align-items-center">
                            <a href="{{ asset('uploads/daily_activity/' . $lampiran->file) }}" target="_blank" class="btn btn-info btn-sm">
                    ...
                    <h5><b>Tambah Lampiran Baru</b></h5>
                ...
                <div class="card-footer">
                    ...
                    <a href="{{ route('mro.daily-activity.index') }}" class="btn btn-secondary">Kembali</a>
                </div>
            </div>
        </form>

        {{-- FORM TERSEMBUNYI UNTUK PROSES DELETE LAMPIRAN --}}
        <form id="deleteLampiranForm" action="" method="POST" style="display: none;">
            @csrf
            @method('DELETE')
        </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            /* --------------------------------------------------------------------------
             * PERSONIL & LAMPIRAN INPUT DYNAMIC
             * -------------------------------------------------------------------------- */
        ...
        document.addEventListener('click', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    // Menggunakan route helper laravel dan mengganti placeholder ID
                    let routeUrl = "{{ route('mro.daily-activity.lampiran.destroy', ':id') }}";
                    form.action = routeUrl.replace(':id', btn.dataset.id);
